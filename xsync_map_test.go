...
func TestNewXsyncMap(t *testing.T) {
	logger := ctxd.LoggerMock{}
	st := stats.TrackerMock{}

	func() {
		c := cache.NewXsyncMap(func(config *cache.Config) {
...
func (cl xsyncMapBaseline) make(b *testing.B, cardinality int) (cacheLoader, string) {
	...
	return xsyncMapBaseline{
		c:           c,
		cardinality: cardinality,
	}, "xsync.Map-base"
}

func (cl xsyncMapBaseline) run(b *testing.B, cnt int, writeEvery int) {
	b.Helper()
