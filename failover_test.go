...
func backends(options ...func(*cache.Config)) []cache.ReadWriter {
...
func TestFailover_Get_ErrorConcurrency(t *testing.T) {
	for _, be := range backends() {
		...
		t.Run(fmt.Sprintf("%T", be), func(t *testing.T) {
			...
			ctx := context.Background()
			sc := cache.NewFailover(func(cfg *cache.FailoverConfig) {
				cfg.Backend = be
			})
...
func TestFailover_Get_keyLock(t *testing.T) {
	for _, be := range backends() {
		...
		t.Run(fmt.Sprintf("%T", be), func(t *testing.T) {
			...
			ctx := context.Background()
			sc := cache.NewFailover(cache.FailoverConfig{SyncUpdate: true, Backend: be}.Use)
