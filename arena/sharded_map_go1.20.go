...
var (
	_ cache.ReadWriterOf[any] = &shardedMapOf[any]{}
	_ cache.Deleter           = &shardedMapOf[any]{}
	_ cache.WalkerOf[any]     = &shardedMapOf[any]{}
)
...
type hashedBucketOf[V any] struct {
	sync.RWMutex
	data map[uint64]*cache.TraitEntryOf[V]
}

type shardedMapOf[V any] struct {
	hashedBuckets []hashedBucketOf[V]

	t *cache.TraitOf[V]
}

// NewShardedMapOf creates an instance of in-memory cache with optional configuration.
func NewShardedMapOf[V any](options ...func(cfg *cache.Config)) *ShardedMapOf[V] {
	c := &shardedMapOf[V]{}
	C := &ShardedMapOf[V]{
		shardedMapOf: c,
	}
	...
	a := arena.NewArena()
	c.hashedBuckets = arena.MakeSlice[hashedBucketOf[V]](a, shards, shards)

	for i := 0; i < shards; i++ {
		c.hashedBuckets[i].data = make(map[uint64]*cache.TraitEntryOf[V])
	}
	...
	if cfg.EvictionStrategy != cache.EvictMostExpired {
...
func (c *shardedMapOf[V]) Store(key []byte, val V) {
	...
	b.data[h] = &cache.TraitEntryOf[V]{V: val, K: k}
}
...
func (c *shardedMapOf[V]) Write(ctx context.Context, k []byte, v V) error {
	...
	ttl, expireAt := expireAt(ctx, c.t.Trait)

	b.data[h] = &cache.TraitEntryOf[V]{V: v, K: key, E: expireAt}
...
func (c *shardedMapOf[V]) Walk(walkFn func(e cache.EntryOf[V]) error) (int, error) {
	n := 0
...
func (c *ShardedMapOf[V]) WalkDumpRestorer() cache.WalkDumpRestorer {
	cc := *c
...
// Walk walks cached entries.
func (c *shardedMapLegacyWalkerOf[V]) Walk(walkFn func(e cache.Entry) error) (int, error) {
	n := 0
...
func (c *ShardedMapOf[V]) Dump(w io.Writer) (int, error) {
	...
	return c.Walk(func(e cache.EntryOf[V]) error {
...
func (c *shardedMapOf[V]) evictMostExpired(evictFraction float64) int {
	return c.evictLeast(evictFraction, func(i *cache.TraitEntryOf[V]) int64 {
...
func (c *shardedMapOf[V]) evictLeast(evictFraction float64, val func(i *cache.TraitEntryOf[V]) int64) int {
	cnt := 0
